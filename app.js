...
async function loadAdminUsers() {
  ...
  try { const data = await authRequest('/api/auth/users'); list.replaceChildren(...(data.users || []).map((user) => { const row = document.createElement('div'); row.className = 'user-list-item'; row.innerHTML = `<span>${escapeHtml(user.username)}</span><small>${user.role === 'admin' ? '管理员' : '普通账户'}</small>`; return row; })); }
